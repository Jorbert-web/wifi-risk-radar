...
function buildRecommendations(answers, isp) {
  ...
  if (answers.encryption !== 'WPA3') {
    recs.push({ category: 'Encryption', severity: answers.encryption === 'WEP' || answers.encryption === 'OPEN' ? 'critical' : 'high',
      ...
      detail: answers.encryption === 'WEP'
        ? 'WEP was deprecated in 2004. It can be cracked in under 60 seconds using Aircrack-ng. Navigate to Wireless Settings → Security Mode → WPA3.'
        : answers.encryption === 'OPEN'
        ? 'An open network broadcasts all traffic unencrypted. Enable WPA3 encryption immediately.'
        : 'WPA has known TKIP vulnerabilities. Upgrade to WPA3 or at minimum WPA2-AES.',
    });
  }
  ...
  if (answers.adminCreds !== 'changed') {
    recs.push({ category: 'Admin Credentials', severity: answers.adminCreds === 'default' ? 'critical' : 'high',
      ...
      detail: `${answers.adminCreds === 'default'
        ? `Default credentials for ${isp} routers (${hint.defaultUser} / ${hint.defaultPass}) are publicly documented.`
        : 'Only changing the Wi-Fi password leaves your admin panel exposed.'
  ...
  if (['weak', 'none'].includes(answers.passphrase)) {
    recs.push({ category: 'Passphrase Strength', severity: answers.passphrase === 'none' ? 'critical' : 'high',
      ...
      detail: 'Use 16+ characters with uppercase, lowercase, numbers, and symbols. Avoid names, birthdays, or dictionary words.',
    });
  }
  ...
  if (answers.firmware !== 'upToDate') {
    recs.push({ category: 'Firmware', severity: answers.firmware === 'outdated' ? 'high' : 'medium',
      ...
      detail: 'Unpatched firmware is a primary vector for remote exploitation. Check ISP app (GlobeOne / MyPLDT) or admin → Firmware Upgrade.',
    });
  }
  ...
  if (answers.wps === 'enabled') {
    recs.push({ category: 'WPS', severity: 'high',
      ...
      detail: 'The Pixie Dust Attack exploits a WPS PIN flaw recovering the passphrase in under 30 seconds. Disable in admin → Wireless → WPS.',
    });
  }
  ...
  if (answers.guestNetwork === 'shared') {
    recs.push({ category: 'Guest Network', severity: 'medium',
      ...
      detail: 'Without isolation, guest devices can attack your smart home devices and computers. Enable AP Isolation in admin → Wireless → Guest Network.',
    });
  }
